...
void Muttaqin()
{
    ...
    map<char, vector<int>> pos_str, pos_temp;
    for (int i = 0; i < n; i++)
    {
        pos_str[str[i]].push_back(i);
        pos_temp[temp[i]].push_back(i);
    }
    vector<int> permutation(n);
    for (char ch = 'a'; ch <= 'z'; ch++)
    {
        for (int i = 0; i < pos_str[ch].size(); i++)
        {
            permutation[pos_temp[ch][i]] = pos_str[ch][i];
        }
    }
    pbds<int> pb;
    long long res = 0;
    for (int i = n - 1; i >= 0; i--)
    {
        res += pb.order_of_key(permutation[i]);
        pb.insert(permutation[i]);
    }
    cout << res << endl;
}
